...
int load_sequence_data(int seqnum, void *memptr) // 8003980C
{
	...
	/* PS3: redondeado a 8 para que la secuencia siguiente arranque alineada
	 * (track_data tiene punteros de 64 bits) */
	return (int)(((char *)pmem - (char *)memptr + 7) & ~7);
}
...
int wess_seq_loader_sizeof(void *input_pm_stat, char *seqfile) // 80039C20
{
	...
	if (ref_pm_stat != NULL)
	{
		...
		/* PS3: el directorio del archivo son entradas de 16 bytes (puntero de
		 * 32 bits); en memoria sequence_data es mas grande. */
		size = sfile_hdr.sequences * sizeof(sequence_data);
	}
...
int wess_seq_loader_init(void *input_pm_stat, char *seqfile, enum OpenSeqHandleFlag flag, char *memory_pointer, int memory_allowance) // 80039CE4
{
	...
	if (ref_pm_stat != NULL)
	{
		...
		if (!decomp_type)
		{
			/* PS3: entrada por entrada (16 bytes en el archivo:
			 * seq_header de 12 + puntero de 32 bits sin usar) */
			int i;
			if ((unsigned int)memory_allowance < sfile_hdr.sequences * sizeof(sequence_data) ||
			    readbytes != sfile_hdr.sequences * 16)
			{
				err(SEQLOAD_FREAD);
				return (0);
			}
			for (i = 0; i < sfile_hdr.sequences; i++)
			{
				sequence_data *sd = ref_pm_stat->pmod_info->pseq_info + i;
				unsigned char raw[16];
				if (module_read(raw, 16, fp_seq_file) != 16)
				{
					err(SEQLOAD_FREAD);
					return (0);
				}
				memcpy(&sd->seq_hdr, raw, sizeof(seq_header));
				sd->ptrk_info = NULL;
			}
...
int wess_seq_sizeof(int seqnum) // 80039EC8
{
	...
	if (seq_loader_enable)
	{
		...
		return (int)(((psq_info->seq_hdr.trkinfolength + 7) & ~7) + numtracks);
	}
	return 0;
}

...
{
	/* PS3: el original usaba __ll_mul/__ull_div del N64 con asm MIPS.
	 * Es ((qpm << 16) * ppq / 60) / ips en 64 bits. */
	int64_t v = (int64_t)qpm * (1 << 16);
	v *= ppq;
	v = (int64_t)((uint64_t)v / 60u);
	v = (int64_t)((uint64_t)v / (uint64_t)(int64_t)ips);
	return (u32)v;
}

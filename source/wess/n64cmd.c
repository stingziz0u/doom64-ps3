...
{
	g_wddloc = (uintptr_t)wdd_location;
}
...
void N64_DriverInit (track_status *ptk_stat) // 80037DA8
{
	...
	pmem += (unsigned int)(ppgd->pat_grp_hdr.drummaps * 4);	/* PS3: punteros de 4 bytes en el archivo */
	//pmem = ((unsigned int)pmem + 7) & ~7;	/* phrase align everything */
#if _ALIGN8_ == 1
	//force align to word boundary because previous size adjust
	//may wind up with odd address
	...
	for (vt = 0; vt < ppgd->pat_grp_hdr.patchinfo; vt++)
	{
		...
		/* PS3: en vez de punteros (de 32 bits en el N64) se guardan offsets
		 * e indices; n64synth.c los resuelve con los *base de arriba. */
		sample->wave.flags = 0;        // base = offset en el WDD

		sample->pitch = (s32)(sample->wave.loop);//move pitch value

		sample->wave.loop = sample->wave.book;  // indice en la tabla de loops, o 0xFFFFFFFF
		if (sample->wave.type != AL_RAW16_WAVE)
			sample->wave.book = vt;            // ADPCM: el codebook de esta muestra
	}
}

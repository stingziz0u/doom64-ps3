...
/* PS3: ticker del MiniLoop activo, para que el mapeo del DS3 sepa en que
 * pantalla esta (guardar/cargar esperan otro boton que los menus). */
int (*ps3_cur_ticker)(void) = NULL;

int MiniLoop(void (*start)(void), void (*stop)(int), int (*ticker)(void), void (*drawer)(void))
{
	int (*prev_ticker)(void) = ps3_cur_ticker;

...
/* ---------------------------------------------------------------------------
 * Swap32 / SwapShort: CONTENIDO de los lumps, no la tabla del WAD.
 *
 * Los dos son distintos de LONGSWAP/LITTLESHORT (kos.h), que atienden la
 * tabla de lumps y las structs de mapa. Estos atienden los datos que salen
 * DERECHO del ROM de Nintendo 64: spriteN64_t, gfxN64_t, textureN64_t, las
 * paletas que van pegadas a esos headers, y los demos. El wadtool reempaqueta
 * el ROM pero NO toca el contenido de los lumps, asi que adentro siguen en
 * BIG-ENDIAN, que es el orden nativo de la N64.
 *
 * De ahi que el original swapee SIEMPRE: el Dreamcast es little-endian y
 * tiene que dar vuelta esos campos. La PS3 es big-endian, o sea que los lee
 * como estan, y swapear seria justamente romperlos.
 *
 * Los 27 usos en el motor (r_data, r_phase1/2/3, p_spec, m_main, d_screens)
 * son todos sobre datos de N64, sin una sola excepcion, asi que la condicion
 * va aca y no en cada lugar.
 *
 * OJO con la asimetria, que es la parte contraintuitiva de todo el port:
 *   tabla del WAD y structs de mapa  -> little-endian -> en PS3 SE swapea
 *   contenido de los lumps (N64)     -> big-endian    -> en PS3 NO se swapea
 * Son dos cosas distintas dentro del mismo archivo porque las escribio el
 * wadtool en un caso y la N64 en el otro.
 * ------------------------------------------------------------------------- */
#if defined(__BYTE_ORDER__) && (__BYTE_ORDER__ == __ORDER_BIG_ENDIAN__)

static inline uint32_t Swap32(uint32_t val) { return val; }
static inline short    SwapShort(short dat) { return dat; }
...
#define SETTINGS_SAVE_VERSION 4   /* 4: + Gamma (PS3) */
#ifndef PS3_GAMMA_LEVELS
#define PS3_GAMMA_LEVELS 9        /* 0.50 0.75 OFF 1.25 1.50 1.75 2.00 2.50 3.00 */
...
typedef struct doom64_settings_s {
	...
	int Interpolate;
	int VmuDisplay;
	int Gamma;          /* PS3: nivel de correccion de gamma */
} doom64_settings_t;
...
#define MAX_CACHED_SPRITES 1024 /* PS3: 256 en doom64-dc */
...
#define MEM_HEAP_SIZE (32 * 1024 * 1024) /* PS3: el de doom64-dc era 0x528000 (~5 MB, para los 16 MB de la Dreamcast) */
...
/* PS3: el wadtool escribe los headers spriteDC_t en little-endian (formato
 * Dreamcast). Leerlos a mano, byte por byte, anda igual en LE y en BE. */
static inline int d64_le16u(const void *p)
{
    const uint8_t *b = (const uint8_t *)p;
    return (int)(b[0] | (b[1] << 8));
}
#define SDC_WIDTH(d)  (d64_le16u(&((spriteDC_t *)(d))->width))
#define SDC_HEIGHT(d) (d64_le16u(&((spriteDC_t *)(d))->height))
#define SDC_XOFFS(d)  ((int)(short)d64_le16u(&((spriteDC_t *)(d))->xoffs))
#define SDC_YOFFS(d)  ((int)(short)d64_le16u(&((spriteDC_t *)(d))->yoffs))

...
void P_BuildMove(player_t *player) // 80022154
{
	...
	if (buttons & cbutton->BT_STRAFE) {
		...
		if (cbutton->BT_LEFT & buttons) {
			...
			if (sensitivity != 0 && !demoplayback) {
				/* PS3: escala mas suave. El original iba de 17 a 1033
				 * (x60) y desde el 30% ya era injugable con el stick del
				 * DS3; ahora va de 17 a ~423: el maximo es mas o menos el
				 * 40% de antes. Los demos del N64 (abajo) siguen igual. */
				sensitivity = (((menu_settings.M_SENSITIVITY * 16) / 5) + 17) *
								sensitivity;

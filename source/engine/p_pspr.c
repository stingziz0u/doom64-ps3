...
/* PS3: con el audio del N64 (WESS) el zumbido del plasma es la secuencia
 * sfx_electric, que se repite sola (como en el N64). */
...
void P_StartElectricLoop(void)
{
	...
	if (PS3_Wess_Ready()) {
		S_StartSound(players[0].mo, sfx_electric);
		plasma_loop_channel = 0x7fff;
		return;
	}

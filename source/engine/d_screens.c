...
int D_TitleMap(void)
{
	...
	/* PS3: sin "mantener START para administrar partidas" al arrancar:
	 * choca con el mismo atajo de webMAN y puede colgar la consola. Las
	 * partidas se manejan por FTP/USB en /dev_hdd0/data/doom64/. */
...
void D_DrawWarning(void)
{
	...
	ST_DrawString(-1, 60, "no controller", 0xffffffff, ST_ABOVE_OVL);
	ST_DrawString(-1, 80, "is connected.", 0xffffffff, ST_ABOVE_OVL);
	ST_DrawString(-1, 120, "please connect your", 0xffffffff, ST_ABOVE_OVL);
	ST_DrawString(-1, 140, "controller and", 0xffffffff, ST_ABOVE_OVL);
	ST_DrawString(-1, 160, "press the ps button.", 0xffffffff, ST_ABOVE_OVL);
...
void D_DrawNoPak(void)
{
	...
	ST_DrawString(-1, 40, "save folder missing.", 0xffffffff, ST_ABOVE_OVL);
	ST_DrawString(-1, 60, "your game cannot", 0xffffffff, ST_ABOVE_OVL);
	ST_DrawString(-1, 80, "be saved.", 0xffffffff, ST_ABOVE_OVL);
	ST_DrawString(-1, 120, "check the free space", 0xffffffff, ST_ABOVE_OVL);
	ST_DrawString(-1, 140, "on the hdd and", 0xffffffff, ST_ABOVE_OVL);
	ST_DrawString(-1, 160, "restart the game.", 0xffffffff, ST_ABOVE_OVL);

...
#define M_TXT44 "Manage Saves"
#define M_TXT45 "Do not save"
#define M_TXT46 "Try again"
#define M_TXT47 "Create save file"
...
// Credits
#define M_TXT69 "CREDITS" // [Immorpher] Credits
#define M_TXT70 "DOOM 64 DC BY: JNMARTIN84" // [Immorpher] Credits (PS3: base del port)
...
#define M_TXT97 "FPS Counter:"

#define M_TXT98 "On"
#define M_TXT99 "Medium"
#define M_TXT100 "Strong"

#define M_TXT101 "WIREFRAME"
#define M_TXT102 "Gamma:"

/* PS3: pantalla "Gamepad" propia (compat/ps3_system.c) */
...
menuitem_t Menu_Video[NUM_MENU_VIDEO] = {
	...
	{ 102, 82, 120 }, // Gamma (PS3)
	{ 88, 82, 140 }, // Quality menu
	{ 92, 82, 160 }, // fps menu
	{ 96, 82, 180 }, // interpolate
	{ 6, 82, 200 }, // Return
};
...
void M_ResetSettings(doom64_settings_t *s) {
	...
	s->enable_messages = 1;
	s->M_SENSITIVITY = 68; // PS3: con la escala nueva, el mismo giro que el 27 de antes
...
/* PS3: todos los sliders recorren su rango completo en el mismo tiempo que
 * Brightness (127 pasos de a 1 por llamada). Los de rango mas chico avanzan
 * de a fracciones; el primer toque siempre mueve un paso. Devuelve 1 si el
 * valor cambio (y ya hizo el sonido). */
static float s_slider_acc = 0.0f;
static int s_slider_held = 0;
static int s_slider_snd = 0;

static int M_SliderMove(int *val, int max, unsigned int buttons)
{
	int dir = (buttons & PAD_RIGHT) ? 1 : ((buttons & PAD_LEFT) ? -1 : 0);
	int old = *val, steps;
	...
	if (!s_slider_held) {
		...
		s_slider_acc += (float)max / 127.0f;
		steps = (int)s_slider_acc;
		if (steps <= 0)
			return 0;
		s_slider_acc -= (float)steps;
	}
	*val += dir * steps;
	if (*val < 0) *val = 0;
	if (*val > max) *val = max;
...
/* PS3: opciones de varios valores (On/Off, Low/Medium/Ultra...): X o derecha
 * = siguiente, izquierda = anterior. 0 = no se toco. */
static int M_CycleDir(unsigned int buttons, unsigned int oldbuttons, int truebuttons)
{
	unsigned int pressed = (buttons ^ oldbuttons) & buttons;
	if (pressed & PAD_LEFT)
		return -1;
	if (pressed & PAD_RIGHT)
		return 1;
	return truebuttons ? 1 : 0;
}
		...
		if ((buttons & PAD_START) && !(oldticbuttons[0] & PAD_START)) {
			...
			case 0: // Gamepad
				if (truebuttons) {
					...
					MenuCall = PS3_ControlPadDrawer;
					cursorpos = 0;
					linepos = 0;

					MiniLoop(M_FadeInStart, M_FadeOutStart, PS3_ControlPadTicker, M_MenuGameDrawer);
			...
			case 5: // [Immorpher] Updated Restart Level
				if (truebuttons) {
					...
					// PS3: tambien circulo/START (ga_exit con el cursor en
					// cualquier dificultad); antes reiniciaba el nivel.
					if (exit == ga_exit) { 
						M_RestoreMenuData((exit == ga_exit));
						return ga_nothing;
					}
			...
			case 7: // Music Volume
				if (M_SliderMove(&menu_settings.MusVolume, 100, buttons)) {
			...
			case 8: // Sound Volume
				if (M_SliderMove(&menu_settings.SfxVolume, 75, buttons)) {
			...
			case 9: // Brightness
				if (M_SliderMove(&menu_settings.brightness, MAX_BRIGHTNESS, buttons)) {
					P_RefreshBrightness();
					return ga_nothing;
				}
				break;
			...
			case 12: // Autorun
				if (M_CycleDir(buttons, oldbuttons, truebuttons)) {
			...
			case 14: // New Game
				if (truebuttons)
				{
					...
					if (extra_episodes) {
						...
						if (extra_episodes == 1) {
							/* PS3: con un solo episodio extra doom64-dc daba
							 * por hecho que era Knee Deep; aca puede ser
							 * Lost Levels (el WAD del remaster, sin Knee Deep). */
							Menu_Episode[1].casepos = kneedeep_only ? 87 : 86;
			...
			case 23: // WARP TO LEVEL
				if (buttons ^ oldbuttons) {
					if (buttons & PAD_LEFT) {
						...
						if (extra_episodes == 1 && !kneedeep_only) {
							/* PS3: solo Lost Levels, no hay map41.. */
							if (m_actualmap > LOST_LASTLEVEL - 1) {
								m_actualmap = LOST_LASTLEVEL - 1;
							}
						} else if (extra_episodes) {
							if (m_actualmap > 49) {
								m_actualmap = 49;
							}
			...
			case 27: // WEAPONS
				if (truebuttons) {
					...
					players[0].artifacts |= 7; // PS3: las 3 llaves demonio (Unmaker)
			...
			case 34: // [Immorpher] HUD opacity
				if (M_SliderMove(&menu_settings.HUDopacity, 255, buttons))
					return ga_nothing;
				break;
			...
			case 43: // Sensitivity
				if (M_SliderMove(&menu_settings.M_SENSITIVITY, 127, buttons))
					return ga_nothing;
				break;
			...
			case 52: // Motion Bob (0..16 en fixed point, de a 1/2)
				{
					int bob = menu_settings.MotionBob / 0x8000;
					if (M_SliderMove(&bob, 32, buttons)) {
						menu_settings.MotionBob = bob * 0x8000;
						return ga_nothing;
					}
				}
				break;
			...
			case 64: // HUDmargin
				if (M_SliderMove(&menu_settings.HUDmargin, 20, buttons))
					return ga_nothing;
				break;
			...
			case 95:
				{
					...
					if (d) {
					S_StartSound(NULL, sfx_switch2);
					menu_settings.Rumble = (menu_settings.Rumble + NUM_RUMBLEPAKS + d) % NUM_RUMBLEPAKS;
			...
			case 102: // Gamma (PS3): X o derecha = subir, izquierda = bajar
				{
					int d = M_CycleDir(buttons, oldbuttons, truebuttons);
					if (d) {
						menu_settings.Gamma = (menu_settings.Gamma + PS3_GAMMA_LEVELS + d) % PS3_GAMMA_LEVELS;
						S_StartSound(NULL, sfx_switch2);
						return ga_nothing;
					}
				}
			...
			case 97: // VMU Display mode (PS3: contador de FPS)
				{
					int d = M_CycleDir(buttons, oldbuttons, truebuttons);
					if (d) {
					S_StartSound(NULL, sfx_switch2);
					menu_settings.VmuDisplay = (menu_settings.VmuDisplay + 3 + d) % 3;
					force_vmu_refresh = 1;
					return ga_nothing;
					}
				}
...
void M_MenuTitleDrawer(void)
{
	...
	if (MenuItem == Menu_Game) {
		...
		ST_DrawString(-1, 200, "press start to resume", text_alpha | 0xffffff00, ST_ABOVE_OVL);
		...
		ST_DrawString(-1, 20, "Delete Save File?", text_alpha | 0xc0000000, ST_ABOVE_OVL);
	} else if (MenuItem == Menu_ControllerPakBad) {
		ST_DrawString(-1, 20, "Save File Bad", text_alpha | 0xc0000000, ST_ABOVE_OVL);
	} else if (MenuItem == Menu_ControllerPakFull) {
		ST_DrawString(-1, 20, "Save Folder Full", text_alpha | 0xc0000000, ST_ABOVE_OVL);
	} else if (MenuItem == Menu_CreateNote) {
		ST_DrawString(-1, 20, "Create Save File?", text_alpha | 0xc0000000, ST_ABOVE_OVL);
...
{
	/* PS3: lista propia (maximo 37 caracteres por linea desde x=20).
	 * red = titulo de seccion. */
	static const struct { int y, red; const char *txt; } cr[] = {
		{  48, 0, "DOOM 64 DC: JNMARTIN84" },
		{  58, 0, "DOOM 64 RE, D64TOOL: ERICK194" },
		{  68, 0, "MERCILESS EDITION, XE: IMMORPHER" },
		{  78, 0, "KNEE DEEP: Z0K, ANDREW HULSHULT" },
		{  88, 0, "RSX CODE: IOQUAKE3-PS3, MAYO1970" },
		{  98, 0, "PSL1GHT, PS3TOOLCHAIN: PS3DEV" },
		{ 112, 1, "DOOM 64 DC THANKS" },
		{ 122, 0, "FALCO GIRGIS, PAUL CERCUEIL, QUZAR," },
		{ 132, 0, "SWAT, BBHOODSTA, KAZADE, ERIK5249," },
		{ 142, 0, "LOBOTOMY, MITTENS," },
		{ 152, 0, "STRIKERTHEHEDGEFOX," },
		{ 162, 0, "#KALLISTIOS/SIMULANT DISCORD" },
		{ 172, 0, "AND EVERY TESTER" },
		{ 186, 1, "DOOM 64" },
		{ 196, 0, "MIDWAY, ID SOFTWARE, AUBREY HODGES" },
	};
	int i;

	ST_DrawString(-1, 20, "Credits", text_alpha | 0xc0000000, ST_ABOVE_OVL);
	ST_Message(40, 33, "DOOM64-PS3: PORT OF DOOM 64 DC", text_alpha | 0xc0000000, ST_ABOVE_OVL);

	for (i = 0; i < (int)(sizeof(cr) / sizeof(cr[0])); i++)
		ST_Message(20, cr[i].y, (char *)cr[i].txt,
			   text_alpha | (cr[i].red ? 0xc0000000 : 0xffffff00), ST_ABOVE_OVL);
}
...
void M_VideoDrawer(void)
{
	...
	for (i = 0; i < NUM_MENU_VIDEO; i++) {
		...
		if (casepos == 102) { // gamma (PS3)
			static char *gamma_txt[PS3_GAMMA_LEVELS] = {
				"0.50", "0.75", "Off", "1.25", "1.50", "1.75", "2.00", "2.50", "3.00"
			};
			int gl = menu_settings.Gamma;
			if (gl < 0 || gl >= PS3_GAMMA_LEVELS) gl = PS3_GAMMA_OFF;
			text = gamma_txt[gl];
		} else if (casepos == 96) { // interpolate
...
void M_DisplayDrawer(void)
{
	...
	for (i = 0; i < itemlines; i++) {
		...
		if (text)
			ST_DrawString(item->x + 165 /* PS3: "FPS Counter:" es largo */, item->y, text, text_alpha | 0xc0000000, ST_ABOVE_OVL);
...
void M_ControllerPakDrawer(void)
{
	...
	int i, j;
	uint8_t idx;
	ST_DrawString(-1, 20, "Saves", text_alpha | 0xc0000000, ST_ABOVE_OVL);

	if (FilesUsed == -1) {
		if ((MenuAnimationTic & 2) != 0)
			ST_DrawString(-1, 114, "Save folder missing!", text_alpha | 0xc0000000, ST_ABOVE_OVL);
		...
		ST_DrawString(-1, 200, "press circle to exit", text_alpha | 0xffffff00, ST_ABOVE_OVL);
		ST_DrawString(-1, 215, "press square to delete", text_alpha | 0xffffff00, ST_ABOVE_OVL);
	}
}

extern int32_t Pak_Size;

/* PS3: ultimo slot usado (al guardar o al cargar). Las pantallas de
 * guardar/cargar arrancan ahi en vez de en el primer slot vacio/lleno. */
static int ps3_last_slot = -1;

static void ps3_cursor_to(int slot, int size)
{
	cursorpos = slot;
	if (!(size < (slot + 6)))
		linepos = slot;
...
void M_SavePakStart(void)
{
	...
	if (ret == 0) {
		if (I_ReadPakFile() == 0) {
			...
			if (ps3_last_slot >= 0 && ps3_last_slot < size)
				ps3_cursor_to(ps3_last_slot, size);
		}
	} else {
		FilesUsed = -1;
	}
...
int M_SavePakTicker(void)
{
	...
	// PS3: una vez guardado (last_ticon != 0) el cursor queda fijo durante
	// la espera de 2 segundos, para que no parezca que se guardo en otro slot.
	if (!(buttons & ALL_JPAD) || last_ticon != 0) {
		f_m_vframe1 = 0;
	} else {
		f_m_vframe1 -= f_vblsinframe[0];
...
void M_SavePakDrawer(void)
{
	...
	if (FilesUsed == -1) {
		if (MenuAnimationTic & 2) {
			ST_DrawString(-1, 100, "Save folder missing!", 0xc00000ff, ST_ABOVE_OVL);
		...
		ST_DrawString(-1, 195, "press circle to exit", text_alpha | 0xffffff00, ST_ABOVE_OVL);
		// changed to z trigger (A button)
		ST_DrawString(-1, 210, "press cross to save", text_alpha | 0xffffff00, ST_ABOVE_OVL);
	}
...
void M_LoadPakStart(void)
{
	...
	if (ps3_last_slot >= 0 && ps3_last_slot < size && Pak_Data[ps3_last_slot * 32])
		ps3_cursor_to(ps3_last_slot, size);
...
void M_LoadPakDrawer(void)
{
	...
	ST_DrawString(-1, 195, "press circle to exit", text_alpha | 0xffffff00, ST_ABOVE_OVL);
	ST_DrawString(-1, 210, "press cross to load", text_alpha | 0xffffff00, ST_ABOVE_OVL);
}

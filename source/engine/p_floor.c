	...
	while ((secnum = P_FindSectorFromLineTag(line->tag, secnum)) >= 0) {
		...
		case customFloor:
			...
			floor->floordestheight = floor->sector->floorheight +
						 (macrointeger * FRACUNIT);
			break;
		case customFloorToHeight:
			floor->direction = ((macrointeger * FRACUNIT) >
...
int EV_SplitSector(line_t *line, boolean sync) // 80014234
{
	...
	while ((secnum = P_FindSectorFromLineTag(line->tag, secnum)) >= 0) {
		...
		split->ceildest =
			sec->ceilingheight + (macrointeger * FRACUNIT);

		if (sync) {
			split->flrdest =
				sec->floorheight + (macrointeger * FRACUNIT);

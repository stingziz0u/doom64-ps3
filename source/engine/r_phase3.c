...
void R_RenderThings(subsector_t *sub)
{
	...
	if (vissprite_p) {
		...
		while (vissprite_p) {
			...
			width = SDC_WIDTH(data);
			height = SDC_HEIGHT(data);
			...
			if (flip) {
				xx = thingx + (SDC_XOFFS(data) * viewsin);

				xpos2 = (xx) >> FRACBITS;
				xpos1 = (xx - (width * viewsin)) >> FRACBITS;

				yy = thingy - (SDC_XOFFS(data) * viewcos);
			...
			ypos = (thingz >> FRACBITS) + SDC_YOFFS(data);
			...
			if ((lump <= 348) || ((lump >= 924) && (lump <= 965))) {
				...
				*hdr_ptr = newhp2v;
				...
				if (external_pal(lump) && thing->info->palette) {
					...
					case 'B':
						// BARO
						PS3_PUT_LE32(lumpname, 0x4F524142);
						break;
					case 'P':
						switch (lumpname[1]) {
						case 'O':
							// ZOMB
							PS3_PUT_LE32(lumpname, 0x424D4F5A);
							break;
						default:
							// PLY1 / PLY2
							PS3_PUT_LE32(lumpname, 0x30594C50 + (thing->info->palette << 24));
							break;
						}
						break;
					case 'S':
						// SPEC
						PS3_PUT_LE32(lumpname, 0x43455053);
						break;
					case 'T':
						// NITE
						PS3_PUT_LE32(lumpname, 0x4554494E);
						break;
					default:
						break;
					}
			...
			bail_evict:
				...
				skip_cached_setup:
					/* PARCHE PS3: el ';' es obligatorio con gcc < C23.
					 * Una etiqueta no puede ir seguida de una declaracion;
					 * necesita una sentencia en el medio, y la sentencia
					 * vacia alcanza. KOS usa un gcc moderno que lo acepta
					 * como extension, ppu-gcc 7.2 no.
					 * Es el UNICO parche al motor en todo el port. */
					;
	...
	for (i = 0; i < NUMPSPRITES; i++, psp++) {
		...
		if ((state = psp->state) != 0) {
			...
			*/
			...
			x = ((psp->sx >> FRACBITS) - SDC_XOFFS(data)) + 160;
			y = ((psp->sy >> FRACBITS) - SDC_YOFFS(data)) + 239;

...
boolean P_TryMove(mobj_t *thing, fixed_t x, fixed_t y) // 80016724
{
	...
	if (latchedmovething) {
		if (thing->flags &
		    MF_MISSILE) { /* missile bash into a monster */
			damage = ((P_Random() & 7) + 1) * thing->info->damage;
			P_DamageMobj(latchedmovething, thing, thing->target,
				     damage);
		} else if (thing->flags &
			   MF_SKULLFLY) { /* skull bash into a monster */
			damage = ((P_Random() & 7) + 1) * thing->info->damage;
...
boolean P_CheckUseHeight(line_t *line) // 80016858
{
	...
	if (flags == ML_SWITCHX08) {
		check = (line->backsector->ceilingheight + rowoffset) +
			(32 * FRACUNIT);
	} else if (flags == ML_CHECKFLOORHEIGHT) {
		check = (line->backsector->floorheight + rowoffset) -
			(32 * FRACUNIT);
	} else if (flags == (ML_CHECKFLOORHEIGHT | ML_SWITCHX08)) {
		check = (line->frontsector->floorheight + rowoffset) +
			(32 * FRACUNIT);
	} else
		return true;
